...
def des_ofb_encrypt(plaintext, key, iv):
    ...
    for i in range(0, len(plaintext_bits), 64):
        keystream = des_encrypt_block(keystream, keys)

        block = plaintext_bits[i:i+64]
        encrypted_block = xor(keystream, block)

        ciphertext_bits.extend(encrypted_block)

    return ciphertext_bits

def des_ofb_decrypt(ciphertext_bits, key, iv):
    key_bits = text_to_bits(key)
    ...
    for i in range(0, len(ciphertext_bits), 64):
        ...
        decrypted_block = xor(keystream, block)
...
ciphertext_bits = des_ofb_encrypt(plaintext, key, iv)
print(f"Ciphertext (bits): {''.join(map(str, ciphertext_bits))}")

decrypted_text = des_ofb_decrypt(ciphertext_bits, key, iv)
